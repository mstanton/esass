...
class LogEntry:
    ...
    def from_dict(cls, data: dict) -> "LogEntry":
        """Create from dictionary, handling common aliases and filtering unknown keys"""
        import dataclasses as _dc

        # Handle common aliases
        if "id" in data and "event_id" not in data:
            data["event_id"] = data["id"]
        if "data" in data and "event_data" not in data:
...
class PatternDefinition:
    ...
    sequence: List[
        str
    ]  # Event type signatures (e.g., "reasoning:git", "tool_usage:git,status")
...
class SkillManifest:
    ...
    def to_dict(self) -> dict:
        ...
        if self.triggers:
            if isinstance(self.triggers[0], str):
                ...
                data["triggers"] = [
                    t.to_dict() if hasattr(t, "to_dict") else t for t in self.triggers
                ]
        return data
    ...
    def from_dict(cls, data: dict) -> "SkillManifest":
        ...
        data_copy = {
            k: v for k, v in data.items() if k in valid_fields or k == "triggers"
        }
        ...
        for t in triggers_data:
            if isinstance(t, str):
                ...
                if len(parts) == 2:
                    triggers.append(
                        TriggerCondition(trigger_type=parts[0], pattern=parts[1])
                    )
                else:
                    # Fallback for malformed strings
                    triggers.append(TriggerCondition(trigger_type="unknown", pattern=t))
