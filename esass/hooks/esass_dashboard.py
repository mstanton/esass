...
class ESASSState:
    ...
    def _update_skills(self):
        ...
        for seq, count in self.sequences.most_common(15):
            if count >= 3:
                ...
                zone = 'trusted' if confidence >= 0.7 else ('learning' if confidence >= 0.3 else 'exploration')
    ...
    def _update_field_metrics(self):
        ...
        if not self.skill_candidates:
            self.trust_radius = 0.1
            self.field_health = 'observing' if self.total_events > 10 else 'initializing'
        else:
...
def format_event_line(event, max_width=45):
    ...
    # Truncate target
    max_target = max_width - 15
    if len(str(target)) > max_target:
        target = str(target)[:max_target-3] + '...'

    return f"{C.DIM}{time_only}{C.RESET} {color}[{icon}]{C.RESET} {target}"
...
def render_dashboard(state: ESASSState, first_render=False):
    ...
    padding = term_width - len(title) - len(timestamp) - 30  # rough estimate for ANSI codes
    ...
    print_line(f"  {C.BOLD}Protection Field{C.RESET}" + " " * (field_width - 14) + f"{C.BOLD}Live Event Stream{C.RESET}")
    print_line()
    ...
    # Legend
    print_line(f"  {C.DIM}[{C.WHITE}{Sym.BLOCK_FULL}{C.DIM}Core {C.GREEN}{Sym.BLOCK_MED}{C.DIM}Trusted {C.YELLOW}{Sym.BLOCK_LIGHT}{C.DIM}Learning {C.BLUE}{Sym.DOT}{C.DIM}Explore]{C.RESET}")
    print_line()
    ...
    health_colors = {
        'initializing': C.DIM, 'observing': C.BLUE,
        'learning': C.YELLOW, 'expanding': C.GREEN, 'stable': C.CYAN
    }
    ...
    # Column 3: Zone Distribution
    zones = state.zone_distribution
    total_skills = sum(zones.values())
    col3 = [
        f"{C.BOLD}Skill Zones{C.RESET}",
        f"",
        f"{C.GREEN}Trusted:{C.RESET}     {zones['trusted']:2}  {render_gauge(zones['trusted'], max(total_skills, 1), 8, C.GREEN)}",
        f"{C.YELLOW}Learning:{C.RESET}    {zones['learning']:2}  {render_gauge(zones['learning'], max(total_skills, 1), 8, C.YELLOW)}",
        f"{C.BLUE}Exploration:{C.RESET} {zones['exploration']:2}  {render_gauge(zones['exploration'], max(total_skills, 1), 8, C.BLUE)}",
        f"",
    ]
    ...
    if state.skill_candidates:
        ...
        for i in range(0, min(6, len(state.skill_candidates)), skills_per_row):
            ...
            for j in range(skills_per_row):
                if i + j < len(state.skill_candidates):
                    ...
                    zone = skill['zone']
                    zone_colors = {'trusted': C.GREEN, 'learning': C.YELLOW, 'exploration': C.BLUE}
                    zc = zone_colors[zone]
        ...
        print_line(f"  {C.DIM}Observing patterns... skills will crystallize as confidence grows{C.RESET}")
    ...
    print_line(f"  {C.BOLD}Raw Event Stream{C.RESET} {C.DIM}(latest {min(15, len(state.recent_events))} of {state.total_events} events){C.RESET}")
    print_line()
    ...
    if events_to_show:
        # Header row
        print_line(f"  {C.DIM}{'TIME':<10} {'TOOL':<8} {'CATEGORY':<15} {'TARGET':<40}{C.RESET}")
        ...
        for event in reversed(events_to_show):
            ...
            category = context.get('category', '-')[:15]
            ...
            print_line(f"  {C.DIM}{time_only}{C.RESET}  {color}[{icon}]{tool:5}{C.RESET} {category:<15} {target}")
    else:
    ...
    print_line(f"  {C.DIM}Press Ctrl+C to exit{last_event_ago} │ Showing {len(events_to_show)}/{len(state.recent_events)} buffered{C.RESET}")
